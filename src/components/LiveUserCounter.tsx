...
const LiveUserCounter: React.FC = () => {
  const { count, loading } = useViewCounter();
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      ...
      >
        <div className={`h-2 w-2 rounded-full ${count > 0 ? 'bg-green-500' : 'bg-red-500'}`}></div>
        <span className="text-xs">{loading ? '--' : count} users online</span>
      </div>
    </div>
  );
};
